...
## 🔍 Como funciona a DFS?
1. **🟢 Início**: Começa no nó inicial, marcando-o como visitado.
2. **🔄 Exploração**: Para cada vizinho do nó atual que não foi visitado, o algoritmo realiza uma chamada recursiva para explorar esse vizinho.
3. **🔙 Retrocesso**: Quando todos os vizinhos de um nó atual são visitados, o algoritmo retorna ao nó anterior para continuar a exploração.
4. **🏁 Conclusão**: O processo continua até que todos os nós alcançáveis sejam visitados.
...
1. **Inicialização:**
   - Começamos definindo o nó de origem, que terá uma distância inicial de 0 para si mesmo, e todos os outros nós terão uma distância inicial de infinito.
